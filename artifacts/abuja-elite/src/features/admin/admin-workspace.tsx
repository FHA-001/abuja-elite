...
export default function AdminWorkspace({loginOnly=false}:{loginOnly?:boolean}){
 ...
 {editor&&<Editor resource={editor.resource} record={editor.record as never} onClose={()=>setEditor(undefined)} onCreate={(r,p)=>save(r,p)} onUpdate={(r,id,p)=>save(r,p,id)} onUploadMedia={upload} createMediaPreview={preview} userId={auth.user?.id||null}/>}
